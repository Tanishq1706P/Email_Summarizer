            ...
            self._llm = LocalOllama(
                OllamaSettings(
                    ...
                    retry_backoff_seconds=float(
                        CFG.get("ollama_retry_backoff_seconds", 0.5)
                    ),
                    offline=bool(CFG.get("offline", True)),
                )
            )
            print(
                f"LLM: {llm_type} model={CFG.get('llm', 'qwen2.5:3b')} host={CFG.get('ollama_host', 'localhost:11434')}"
            )
    ...
    def generate(
        ...
        if self._disk_cache:
            cache_key = self._disk_cache.make_key(
                ...
                user=prompt[:2000],  # Truncate for cache key
            )
            cached = self._disk_cache.get(cache_key)
            if isinstance(cached, dict) and cached:
                return cached

        # Auto-trim prompt for Groq free tier TPM limits (~5000 input tokens)
        if len(prompt) > 4500:
            logger.warning(
                "Prompt too long (%d chars), trimming for Groq TPM limit", len(prompt)
            )
            # Preserve end (likely most actionable) but truncate middle
            head = prompt[:1000]
            tail = prompt[-2000:]
            prompt = (
