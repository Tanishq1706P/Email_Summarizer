...
from pipelines.summarizer.pipeline import (
    EmailSummarizationPipeline as SummarizerPipeline,
)
...
llm_circuit = CircuitBreaker(
    "ollama_llm", failure_threshold=3, recovery_timeout_seconds=60
)
...
class EmailSummarizationPipeline:
    def __init__(self):
        ...
        logger.info(
            ...
            extra={"props": {"learning_enabled": self._cfg.get("learning_enabled")}},
        )
            ...
            if self._cfg.get("learning_enabled"):
                with self._learning_lock:
                    try:
                        ...
                        logger.warning(
                            "Failed to record session in learning store", exc_info=True
                        )
    ...
    def feedback(self, feedback: UserFeedback):
        ...
        try:
            ...
            if self._cfg.get("learning_enabled"):
                with self._learning_lock:
                    try:
                        ...
                        logger.warning(
                            "Failed to record feedback in learning store", exc_info=True
                        )
            ...
            logger.error(
                ...
                extra={"props": {"session_id": feedback.session_id}},
                exc_info=True,
            )
            raise
